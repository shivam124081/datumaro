...
class MergeInstanceSegments(Transform, CliPlugin):
    ...
    def build_cmdline_parser(cls, **kwargs):
        ...
        parser.add_argument(
            "--include-polygons", action="store_true", help="Include polygons"
        )
        return parser
    ...
    def transform_item(self, item):
        ...
        segments = [
            self.merge_segments(i, w, h, self._include_polygons) for i in instances
        ]
...
class Reindex(Transform, CliPlugin):
    ...
    def build_cmdline_parser(cls, **kwargs):
        parser = super().build_cmdline_parser(**kwargs)
        parser.add_argument(
            "-s", "--start", type=int, default=1, help="Start value for item ids"
        )
        return parser
...
class MapSubsets(Transform, CliPlugin):
    ...
    def __init__(self, extractor, mapping=None):
        ...
        if extractor._subsets:
            counts = Counter(
                mapping.get(s, s) or DEFAULT_SUBSET_NAME for s in extractor._subsets
            )
...
class IdFromImageName(Transform, CliPlugin):
    def transform_item(self, item):
        if item.has_image and item.image.path:
            ...
            log.debug(
                "Can't change item id for item '%s': "
...
class Rename(Transform, CliPlugin):
    ...
    def build_cmdline_parser(cls, **kwargs):
        parser = super().build_cmdline_parser(**kwargs)
        parser.add_argument("-e", "--regex", help="Regex for renaming.")
        return parser
    ...
    def transform_item(self, item):
        return self.wrap_item(
            item, id=self._re.sub(self._sub, item.id).format(item=item)
        )
        ...
        for src_index, src_label in enumerate(src_label_cat.items):
            ...
            if dst_index is None:
                dst_index = dst_label_cat.add(
                    dst_label, src_label.parent, src_label.attributes
                )
