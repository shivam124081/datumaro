...
from datumaro.util.annotation_util import max_bbox, find_group_leader, find_instances
...
class TfDetectionApiConverter(Converter):
    ...
    def apply(self):
        ...
        get_label = (
            lambda label_id: label_categories.items[label_id].name
    ...
    def _export_instances(self, instances, width, height):
        xmins = (
            []
        )  # List of normalized left x coordinates of bounding boxes (1 per box)
        xmaxs = (
            []
        )  # List of normalized right x coordinates of bounding boxes (1 per box)
        ymins = []  # List of normalized top y coordinates of bounding boxes (1 per box)
        ymaxs = (
            []
        )  # List of normalized bottom y coordinates of bounding boxes (1 per box)
    ...
    def _make_tf_example(self, item):
        ...
        if not item.has_image:
            raise Exception(
                "Failed to export dataset item '%s': "
