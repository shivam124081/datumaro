...
class _CocoExtractor(SourceExtractor):
    ...
    def _load_person_kp_categories(self, loader):
        ...
        for cat in cats:
            ...
            categories.add(
                label_id=label_id, labels=cat["keypoints"], joints=cat["skeleton"]
            )
    ...
    def _load_annotations(self, ann, image_info=None):
        ...
        if "attributes" in ann:
            try:
                ...
                log.debug(
                    "item #%s: failed to read annotation attributes: %s",
                    image_info["id"],
                    e,
                )
        if "score" in ann:
            ...
            if segmentation and segmentation != [[]]:
                ...
                if isinstance(segmentation, list):
                    ...
                    if img_h == mask_h and img_w == mask_w:
                        rle = mask_utils.frPyObjects([segmentation], mask_h, mask_w)[0]
                    else:
                        log.warning(
                            "item #%s: mask #%s "
