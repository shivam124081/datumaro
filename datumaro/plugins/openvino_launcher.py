...
class OpenVinoImporter(CliPlugin):
    ...
    def build_cmdline_parser(cls, **kwargs):
        ...
        parser.add_argument(
            "--device", default="CPU", help="Target device (default: %(default)s)"
        )
        return parser
    ...
    def copy_model(model_dir, model):
        ...
        shutil.copy(
            model["weights"], osp.join(model_dir, osp.basename(model["weights"]))
        )
...
class InterpreterScript:
    def __init__(self, path):
        ...
        if not callable(process_outputs):
            raise Exception(
                "Can't find 'process_outputs' function in " "the interpreter script"
            )
...
class OpenVinoLauncher(Launcher):
    ...
    def __init__(
        ...
        if not osp.isfile(description):
            raise Exception(
                'Failed to open model description file "%s"' % (description)
            )
        ...
        if not osp.isfile(interpreter):
            raise Exception(
                'Failed to open model interpreter script file "%s"' % (interpreter)
            )
    ...
    def _load_executable_net(self, batch_size=1):
        ...
        self._input_layout = (
            input_type if isinstance(input_type, list) else input_type.shape
        )
        ...
        self._net = self._ie.load_network(
            network=network, num_requests=1, device_name=self._device
        )

    def infer(self, inputs):
        assert (
            len(inputs.shape) == 4
        ), "Expected an input image in (N, H, W, C) format, got %s" % (inputs.shape)
