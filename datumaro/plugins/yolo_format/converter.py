...
class YoloConverter(Converter):
    ...
    def apply(self):
        ...
        label_ids = {
            label.name: idx for idx, label in enumerate(label_categories.items)
        }
        ...
        for subset_name, subset in self._extractor.subsets().items():
            if not subset_name or subset_name == DEFAULT_SUBSET_NAME:
                ...
                log.warn(
                    ...
                    % (
                        subset_name,
                        ", ".join("'%s'" % s for s in YoloPath.SUBSET_NAMES),
                    )
                )
                continue
            ...
            for item in subset:
                ...
                image_paths[item.id] = osp.join(
                    "data", osp.basename(subset_dir), image_name
                )
