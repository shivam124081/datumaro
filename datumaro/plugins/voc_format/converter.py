...
class VocConverter(Converter):
    ...
    def make_dirs(self):
        ...
        cls_subsets_dir = osp.join(
            subsets_dir, VocPath.TASK_DIR[VocTask.classification]
        )
        action_subsets_dir = osp.join(
            subsets_dir, VocPath.TASK_DIR[VocTask.action_classification]
        )
        layout_subsets_dir = osp.join(
            subsets_dir, VocPath.TASK_DIR[VocTask.person_layout]
        )
        ...
        for subset_name, subset in self._extractor.subsets().items():
            ...
            for item in subset:
                ...
                if (
                    ...
                    ET.SubElement(root_elem, "segmented").text = str(
                        int(item_segmented)
                    )
                    ...
                    for new_obj_id, obj in enumerate(main_bboxes):
                        ...
                        if "occluded" in attr:
                            ...
                            ET.SubElement(obj_elem, "occluded").text = "%d" % occluded
                        ...
                        for part_bbox in filter(
                            ...
                            ET.SubElement(part_elem, "name").text = self.get_label(
                                part_bbox.label
                            )
    ...
    def save_action_lists(self, subset_name, action_list):
        ...
        all_actions = set(chain(*(self._get_actions(l) for l in self._label_map)))
        for action in all_actions:
            ann_file = osp.join(
                self._action_subsets_dir, "%s_%s.txt" % (action, subset_name)
            )
            with open(ann_file, "w") as f:
    ...
    def save_class_lists(self, subset_name, class_lists):
        ...
        for label in self._label_map:
            ann_file = osp.join(
                self._cls_subsets_dir, "%s_%s.txt" % (label, subset_name)
            )
            with open(ann_file, "w") as f:
    ...
    def _load_categories(self, label_map_source):
        if label_map_source == LabelmapType.voc.name:
            ...
            labels = self._extractor.categories().get(
    ...
    def _make_label_id_map(self):
        ...
        target_labels = {
            label.name: id
            for id, label in enumerate(self._categories[AnnotationType.label].items)
        }
        id_mapping = {
        ...
        if void_labels:
            log.warning(
                "The following labels are remapped to background: %s"
                % ", ".join(void_labels)
            )
        log.debug(
