...
class _VocExtractor(SourceExtractor):
    def __init__(self, path):
        ...
        label_color = lambda label_idx: self._categories[
            AnnotationType.mask
        ].colormap.get(label_idx, None)
        log.debug(
            "Loaded labels: %s"
            % ", ".join(
...
class VocClassificationExtractor(_VocExtractor):
    def __iter__(self):
        ...
        for item_id in self._items:
            ...
            image = osp.join(
                self._dataset_dir, VocPath.IMAGES_DIR, item_id + VocPath.IMAGE_EXT
            )
            anns = self._parse_annotations(raw_anns, item_id)
            yield DatasetItem(
                id=item_id, subset=self._subset, image=image, annotations=anns
            )
    ...
    def _load_annotations(self):
        ...
        anno_files = [
            s
            for s in dir_items(task_dir, ".txt")
    ...
    def _parse_annotations(self, root_elem):
        ...
        for obj_id, object_elem in enumerate(root_elem.findall("object")):
            ...
            attributes["difficult"] = (
                difficult_elem is not None and difficult_elem.text == "1"
            )

            truncated_elem = object_elem.find("truncated")
            attributes["truncated"] = (
                truncated_elem is not None and truncated_elem.text == "1"
            )

            occluded_elem = object_elem.find("occluded")
            attributes["occluded"] = (
                occluded_elem is not None and occluded_elem.text == "1"
            )
            ...
            if attributes_elem is not None:
                for attr_elem in attributes_elem.iter("attribute"):
                    attributes[attr_elem.find("name").text] = attr_elem.find(
                        "value"
                    ).text
...
class VocSegmentationExtractor(_VocExtractor):
    ...
    def _load_annotations(self, item_id):
        ...
        if osp.isfile(segm_path):
            inverse_cls_colormap = self._categories[
                AnnotationType.mask
            ].inverse_colormap
        ...
        instances_mask = None
        inst_path = osp.join(
            self._dataset_dir, VocPath.INSTANCES_DIR, item_id + VocPath.SEGM_EXT
        )
        ...
        if instances_mask is not None:
            ...
            for instance_id, label_id in instance_labels.items():
                ...
                if label_id is not None:
                    actions = {a: False for a in label_cat.items[label_id].attributes}
            ...
            log.warn(
                "item '%s': has only class segmentation, "
