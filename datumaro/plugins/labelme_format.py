...
class LabelMeExtractor(SourceExtractor):
    ...
    def _parse(self, path):
        ...
        for p in os.listdir(path):
            ...
            item_id = osp.join(
                root.find("folder").text or "", root.find("filename").text
            )
    ...
    def _parse_annotations(cls, xml_root, dataset_root, categories):
        ...
        for obj_elem in xml_root.iter("object"):
            ...
            if poly_elem is not None:
                ...
                mask_path = osp.join(
                    dataset_root, LabelMePath.MASKS_DIR, segm_elem.find("mask").text
                )
                ...
                ann_items.append(
                    Mask(image=mask, label=label, id=obj_id, attributes=attributes)
                )
...
class LabelMeConverter(Converter):
    ...
    def _save_item(self, item, subset_dir, index):
        ...
        for ann in item.annotations:
            ...
            if ann.type == AnnotationType.bbox:
                ...
                ET.SubElement(poly_elem, "username").text = str(
                    ann.attributes.pop("username", "")
                )
                ...
                mask_filename = "%s_mask_%s.png" % (item.id.replace("/", "_"), obj_id)
                save_image(
                ...
                ET.SubElement(segm_elem, "username").text = str(
                    ann.attributes.pop("username", "")
                )
            else:
    ...
    def _paint_mask(mask):
        ...
        return np.array([[0, 0, 0, 0], [255, 203, 0, 153]], dtype=np.uint8)[
