...
TargetKinds = Enum(
    "TargetKinds", ["project", "source", "external_dataset", "inference", "image"]
)
...
def ExternalDatasetTarget(
    kind=TargetKinds.external_dataset,
    test=is_external_source,
    is_default=False,
    name="external dataset path",
):
    return Target(kind, test, is_default, name)


def InferenceTarget(
    kind=TargetKinds.inference,
    test=is_inference_path,
    is_default=False,
    name="inference path",
):
    return Target(kind, test, is_default, name)
...
def ImageTarget(
    kind=TargetKinds.image, test=is_image_path, is_default=False, name="image path"
):
    return Target(kind, test, is_default, name)
...
def target_selector(*targets):
    def selector(value):
        ...
        raise argparse.ArgumentTypeError(
            "Value should be one of: %s" % (", ".join([str(t) for t in targets]))
        )
