...
from ...util.project import load_project, generate_next_name, generate_next_file_name
...
def build_add_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--copy", action="store_true", help="Copy the model to the project"
    )
    parser.add_argument(
        "-n",
        "--name",
...
def add_command(args):
    ...
    if args.name:
        ...
        args.name = generate_next_name(project.config.models, "model", "-", default=0)
    ...
    log.info(
        "Model '%s' with launcher '%s' has been added to project '%s'"
...
def build_run_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-o", "--output-dir", dest="dst_dir", help="Directory to save output"
    )
    parser.add_argument(
        "-m",
        "--model",
...
def run_command(args):
    ...
    if dst_dir:
        ...
        dst_dir = generate_next_file_name("%s-inference" % project.config.project_name)

    project.make_dataset().apply_model(
        save_dir=osp.abspath(dst_dir), model=args.model_name
    )
