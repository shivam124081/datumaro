...
from ...util import CliException, MultilineFormatter, add_subparser, make_file_name
...
def build_import_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser = parser_ctor(
        help="Create project from existing dataset",
        description="""
    ...
    parser.add_argument(
        "--skip-check", action="store_true", help="Skip source checking"
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def import_command(args):
    ...
    if not args.format:
        ...
        try:
            ...
            raise CliException("Importer for format '%s' is not found" % args.format)
...
def build_export_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-e", "--filter", default=None, help="Filter expression for dataset items"
    )
    parser.add_argument(
        "--filter-mode",
...
def export_command(args):
    ...
    if dst_dir:
        ...
        dst_dir = generate_next_file_name(
            "%s-%s" % (project.config.project_name, make_file_name(args.format))
        )
...
def filter_command(args):
    ...
    if not args.dry_run:
        ...
        if dst_dir:
            ...
            dst_dir = generate_next_file_name("%s-filter" % project.config.project_name)
        dst_dir = osp.abspath(dst_dir)
    ...
    dataset.filter_project(save_dir=dst_dir, filter_expr=args.filter, **filter_args)
...
def build_merge_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument("other_project_dir", help="Path to a project")
    parser.add_argument(
        "-o",
        "--output-dir",
        dest="dst_dir",
...
def build_diff_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "other_project_dir", help="Directory of the second project to be compared"
    )
    parser.add_argument(
        "-o",
        "--output-dir",
...
def diff_command(args):
    ...
    if dst_dir:
        ...
        dst_dir = generate_next_file_name(
            "%s-%s-diff"
            % (first_project.config.project_name, second_project.config.project_name)
        )
    dst_dir = osp.abspath(dst_dir)
    ...
    try:
        visualizer = DiffVisualizer(
            save_dir=dst_dir, comparator=comparator, output_format=args.visualizer
        )
...
def build_ediff_parser(parser_ctor=argparse.ArgumentParser):
    parser = parser_ctor(
        help="Compare projects for equality",
        description="""
    ...
    parser.add_argument(
        ...
        "--ignore-field",
        action="append",
        help="Ignore annotation field (repeatable, default: %s)" % _ediff_default_if,
    )
    parser.add_argument(
        "--match-images",
        action="store_true",
    ...
    parser.add_argument(
        "--all", action="store_true", help="Include matches in the output"
    )
    parser.add_argument(
        "-p",
        "--project",
...
def build_transform_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-t", "--transform", required=True, help="Transform to apply to the project"
    )
    parser.add_argument(
        "-o",
        "--output-dir",
...
def transform_command(args):
    ...
    if dst_dir:
        ...
        dst_dir = generate_next_file_name(
            "%s-%s" % (project.config.project_name, make_file_name(args.transform))
        )
    dst_dir = osp.abspath(dst_dir)
    ...
    dataset.transform_project(method=transform, save_dir=dst_dir, **extra_args)
