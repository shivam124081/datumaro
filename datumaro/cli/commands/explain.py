...
def build_parser(parser_ctor=argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-m", "--model", required=True, help="Model to use for inference"
    )
    parser.add_argument(
        "-t",
        "--target",
        default=None,
        help="Inference target - image, source, project " "(default: current dir)",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
    ...
    if args.target[0] == TargetKinds.image:
        ...
        if args.save_dir is not None:
            ...
            for j, heatmap in enumerate(heatmaps):
                ...
                cv2.imshow(file_name + "-heatmap-%s" % j, disp)
            cv2.waitKey(0)
    elif args.target[0] == TargetKinds.source or args.target[0] == TargetKinds.project:
        ...
        for item in dataset:
            ...
            if image is None:
                log.warn(
                    "Dataset item %s does not have image data. Skipping." % (item.id)
                )
                continue
