...
class ProjectTest(TestCase):
    def test_project_generate(self):
        ...
        with TestDir() as test_dir:
            ...
            self.assertEqual(src_config.project_name, result_config.project_name)
            self.assertEqual(src_config.format_version, result_config.format_version)
    ...
    def test_can_import_with_custom_importer(self):
        class TestImporter:
            def __call__(self, path, subset=None):
                return Project({"project_filename": path, "subsets": [subset]})
        ...
        project = Project.import_from(path, importer_name, env, subset="train")
    ...
    def test_can_batch_launch_custom_model(self):
        dataset = Dataset.from_iterable(
            [DatasetItem(id=i, subset="train", image=np.array([i])) for i in range(5)],
            categories=["label"],
        )
        ...
        for item in executor:
            ...
            self.assertEqual(int(item.id), item.annotations[0].attributes["data"])
    ...
    def test_can_do_transform_with_custom_model(self):
        class TestExtractorSrc(Extractor):
            def __iter__(self):
                for i in range(2):
                    yield DatasetItem(
                        id=i, image=np.ones([2, 2, 3]) * i, annotations=[Label(i)]
                    )
        ...
        with TestDir() as test_dir:
            project.make_dataset().apply_model(model=model_name, save_dir=test_dir)
    ...
    def test_cant_merge_different_categories(self):
        ...
        class TestExtractor2(Extractor):
            ...
            def categories(self):
                return {AnnotationType.label: LabelCategories.from_iterable(["b", "a"])}
...
class DatasetFilterTest(TestCase):
    ...
    def test_annotations_filter_can_remove_empty_items(self):
        ...
        filtered = XPathAnnotationsFilter(
            extractor, "/item/annotation[label_id = 2]", remove_empty=True
        )
...
class ConfigTest(TestCase):
    def test_can_produce_multilayer_config_from_dict(self):
        ...
        schema_mid = (
            SchemaBuilder().add("desc", lambda: Config(schema=schema_low)).build()
        )
        schema_top = (
            SchemaBuilder()
            .add(
                "container",
                lambda: DefaultConfig(lambda v: Config(v, schema=schema_mid)),
            )
            .build()
        )
        ...
        value = 1
        source = Config(
            {"container": {"elem": {"desc": {"options": {"k": value}}}}},
            schema=schema_top,
        )
