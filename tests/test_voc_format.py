            ...
            def __iter__(self):
                return iter(
                    [
                        ...
                        DatasetItem(
                            id="2007_000002", subset="test", image=np.ones((10, 20, 3))
                        ),
                    ]
                )
...
class VocConverterTest(TestCase):
                ...
                return iter(
                    [
                        DatasetItem(
                            ...
                            annotations=[
                                Bbox(
                                    2, 3, 4, 5, label=2, attributes={"occluded": True}
                                ),
                                Bbox(
                                    2,
                                    3,
                ...
                return iter(
                    [
                        DatasetItem(
                            ...
                            annotations=[
                                Mask(
                                    image=np.array([[0, 0, 1, 0, 0]]), label=4, group=1
                                ),
                                Mask(
                                    image=np.array([[1, 1, 0, 0, 0]]), label=3, group=2
                                ),
                                Mask(
                                    image=np.array([[0, 0, 0, 1, 0]]), label=3, group=3
                                ),
                            ],
                        ),
                    ]
    ...
    def test_can_save_voc_segm_unpainted(self):
        ...
        with TestDir() as test_dir:
            self._test_save_and_load(
                TestExtractor(),
                partial(
                    VocSegmentationConverter.convert,
                    label_map="voc",
    ...
    def test_can_save_voc_layout(self):
        ...
        with TestDir() as test_dir:
            self._test_save_and_load(
                TestExtractor(),
                partial(VocLayoutConverter.convert, label_map="voc"),
                test_dir,
            )
    ...
    def test_can_save_voc_action(self):
        ...
        with TestDir() as test_dir:
            self._test_save_and_load(
                TestExtractor(),
                partial(
                    VocActionConverter.convert, label_map="voc", allow_attributes=False
                ),
                test_dir,
                target_dataset=DstExtractor(),
            )
