...
class YoloFormatTest(TestCase):
    ...
    def test_can_load_dataset_with_exact_image_info(self):
        ...
        with TestDir() as test_dir:
            ...
            parsed_dataset = YoloImporter()(
                test_dir, image_info={"1": (10, 15)}
            ).make_dataset()
    ...
    def test_relative_paths(self):
        source_dataset = Dataset.from_iterable(
            [
                ...
                DatasetItem(id="subdir1/1", subset="train", image=np.ones((2, 6, 3))),
                DatasetItem(id="subdir2/1", subset="train", image=np.ones((5, 4, 3))),
            ],
            categories={
            ...
            with self.subTest(save_images=save_images):
                with TestDir() as test_dir:
                    YoloConverter.convert(
                        source_dataset, test_dir, save_images=save_images
                    )
                    parsed_dataset = YoloImporter()(test_dir).make_dataset()
...
class YoloImporterTest(TestCase):
    ...
    def test_can_import(self):
        ...
        dataset = Project.import_from(DUMMY_DATASET_DIR, "yolo").make_dataset()
